...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    ...
    if (!authLoading && (!user || !isAdmin)) {
      console.warn(
        `User ${user?.uid} is not authorized for admin page. Role: ${userData?.role}. Redirecting to home.`,
      );
  ...
  const addLeague = async (leagueData: Omit<League, "id">) => {
    // id는 자동 생성되므로 Omit 사용
    if (!user || !isAdmin) {
      // 안전장치: Context 상태로 권한 확인
  ...
  return (
    ...
    <div className="w-full p-4">
      {/* width: full, padding: 4 */}
      {/* user 객체는 useAuth에서 오므로 null이 아님 */}
      <h1 className="text-3xl font-bold mb-6">
        안녕하세요, 관리자 {user.email}님!
      </h1>
      ...
      <div className="mb-8">
        <h2 className="text-2xl font-semibold mb-4">리그 목록</h2>
        {/* 제목 변경 */}
        ...
        >
          <h3 className="text-xl font-medium mb-2">새 리그 추가</h3>
          {/* 부제목 추가 */}
          <div className="mb-4">
            <label
              htmlFor="leagueName"
        ...
        {leagues.length === 0 && !dataLoading ? ( // 데이터 로딩 중이 아닐 때만 "없음" 메시지 표시
          <p className="p-4 bg-white rounded shadow-sm">
            등록된 리그가 없습니다.
          </p>
        ) : (
